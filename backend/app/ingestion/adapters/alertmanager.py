...
class AlertmanagerAdapter:
    ...
    def adapt(self, raw: dict) -> CanonicalEvent:
        ...
        return CanonicalEvent(event_id=event_id(self.source_name, record_id, timestamp, raw), timestamp=timestamp, ingested_at=ingested_at(raw, timestamp), entity_id=entity, modality="alert", event_type=event_type, severity=severity, trace_or_session_id=trace_id(raw, payload), source=self.source_name, source_record_id=record_id, schema_version="1.0", quality_flags=simulated_flags(raw), raw_payload={**payload, **metadata})
