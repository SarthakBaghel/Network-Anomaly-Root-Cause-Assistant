...
class PrometheusAdapter:
    ...
    def adapt(self, raw: dict) -> CanonicalEvent:
        ...
        return CanonicalEvent(event_id=event_id(self.source_name, record_id, timestamp, raw), timestamp=timestamp, ingested_at=ingested_at(raw, timestamp), entity_id=entity, modality="metric", event_type=EVENT_TYPES.get(metric, metric.upper()), severity=0, signal_name=metric, signal_value=value, unit=unit, trace_or_session_id=trace_id(raw, payload), source=self.source_name, source_record_id=record_id, schema_version="1.0", quality_flags=simulated_flags(raw), raw_payload={**payload, **metadata})
