...
from app.db.models import Base, Entity, Event
...
class TestNslKddReader:
    ...
    def test_pipeline_accepts_with_zero_quarantines(self, mem_session, pipeline):
        ...
        accepted, collapsed = _run_batch(
            pipeline, mem_session, reader, raws, dataset_name="NSL-KDD"
        )
...
class TestUnswNb15Reader:
    ...
    def test_pipeline_zero_quarantines(self, mem_session, pipeline):
        ...
        raws = list(reader.records(DATA_ROOT, limit=30))
        accepted, collapsed = _run_batch(
            pipeline, mem_session, reader, raws, dataset_name="UNSW-NB15"
        )
        assert accepted + collapsed == 30
...
class TestLoghubHdfsReader:
    ...
    def test_payload_trace_schema(self):
        reader = LoghubHdfsReader()
        raw = next(reader.records(DATA_ROOT, limit=1))
        p = raw["payload"]
    ...
    def test_trace_id_uses_block_id(self):
        ...
        blk_traces = [
            r["payload"]["trace_id"] for r in raws if r["payload"]["trace_id"].startswith("blk_")
        ]
...
class TestSampleTracesReader:
    ...
    def test_observable_latency_marks_slow_spans_without_using_outcome_fields(self):
        reader = SampleTracesReader()
        slow = [
            r
            for r in reader.records(DATA_ROOT, split="test")
            if r["payload"]["duration_ms"] > r["payload"]["expected_p99_ms"] * 3
        ]
        assert slow
        assert all(item["payload"]["status"] == "error" for item in slow)
        assert all(
            key not in item["payload"]
            for item in slow
            for key in ("nodeLatencyLabel", "graphLatencyLabel", "graphStructureLabel")
        )
    ...
    def test_adapter_accepts_records(self):
        reader = SampleTracesReader()
        adapter = ADAPTERS["simulator.trace"]
        for raw in reader.records(DATA_ROOT, split="test", limit=10):
            clean = {k: v for k, v in raw.items() if k != "_meta"}
            ev = adapter.adapt(clean)
            assert ev.modality.value == "trace"
...
class TestDatasetBridgeRunner:
    ...
    def test_meta_never_reaches_db(self, mem_session):
        ...
        for ev in events:
            ...
            assert "label" not in all_keys, (
                f"label found in raw_payload keys (note: 'labels' is OK): {all_keys}"
            )
