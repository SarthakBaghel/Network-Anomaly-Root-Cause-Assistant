...
export function Sidebar({ activePath }: SidebarProps) {
  ...
  return (
    <aside
      className={`sticky top-0 hidden h-screen shrink-0 flex-col border-r border-border-subtle bg-surface-strong transition-[width] duration-200 md:flex ${
        collapsed ? "w-20" : "w-64"
      }`}
    >
      <div className="flex items-center gap-3 border-b border-border-subtle px-5 py-6">
        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-accent-cyan/40 bg-accent-cyan/10 text-accent-cyan">
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        ...
        >
          ...
          {!collapsed ? <span className="truncate font-data">Current incident</span> : null}
        </a> : null}
      </nav>
      ...
      <button
        ...
        className="m-3 flex items-center justify-center gap-2 rounded-md border border-border-subtle py-2 text-text-secondary transition-colors hover:border-accent-cyan/40 hover:text-accent-cyan"
      >
        {collapsed ? (
